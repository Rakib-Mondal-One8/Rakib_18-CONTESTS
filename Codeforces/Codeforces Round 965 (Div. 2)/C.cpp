...
void solve()
{
	...
	ll tot = accumulate(a.begin(), a.end(), ll(0));
	ll ans = LLONG_MIN;
	loop(i, 0, n - 1) {
		ll val = tot - a[i];
		if (b[i] == 1) {
			ans = max(ans, ll(a[i] + k + ceil(double(val) / 2)));
		}
		if (ok)
			ans = max(ans, ll(a[i] + ceil(double(val + k) / 2)));
		else ans = max(ans, ll(a[i] + ceil(double(val) / 2)));
	}
	cout << ans << nl;
}
int main()
